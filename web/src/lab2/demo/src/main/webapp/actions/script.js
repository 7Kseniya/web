...
function submitForm(board, pointsById, x, y, r) {
    $.ajax({
        type: 'POST',
        url: '/lab2-1.0-SNAPSHOT/controller',
        data: {x: x, y: y, r: r, currentTime: new Date().toLocaleString()},
        success: (data) => {
            $(".right_side #result_table tbody").append(getHtmlRowFromJson(data));
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: (data) => {
            data.forEach(function (element) {
                $(".right_side #result_table tbody").append(getHtmlRowFromJson(element));
    ...
    $('#clearTable').click(function (event) {
        $.ajax({
            ...
            url: '/lab2-1.0-SNAPSHOT/controller',
            success: (data) => {
                $(".right_side #result_table tbody").html(data);
                rInput.each(function () {
